...
: m_sPersonaStateChangedCallback( this, &CAvatarImage::OnPersonaStateChanged )
{
	...
	m_Color = SDK_Color( 255, 255, 255, 255 );
...
// Purpose:
//-----------------------------------------------------------------------------
CAvatarImagePanel::CAvatarImagePanel( vgui2::Panel *parent, const char *name ) : BaseClass( parent, name )
{
	m_bScaleImage = false;
...
{
	if (!entindex) m_pImage->ClearAvatarSteamID();

	uint64 steamID64 = GetSteamID64(g_PlayerSteamId[entindex]);
