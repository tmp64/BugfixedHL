...
void colorpicker::HSVtoRGB(float h, float s, float v, SDK_Color &output)
{
...
void colorpicker::RGBtoHSV(SDK_Color rgb, float &hue, float &sat, float &val)
{
	double delta, min;
	double h = 0, s, v;
...
public:
	...
	virtual void Paint()
	{
		...
		vgui2::surface()->DrawSetColor(SDK_Color(0, 0, 0, 255));
	...
	void SetOldColor(SDK_Color col)
	{
		m_OldColor = col;
	}

	void SetNewColor(SDK_Color col)
	{
		m_NewColor = col;
	}
...
protected:
	SDK_Color m_OldColor, m_NewColor;
	int m_nX = 0, m_nY = 0;
...
CColorPicker::CColorPicker(vgui2::Panel *parent, const char *panelName, const char *title) : vgui2::Frame(parent, panelName)
{
	...
	LoadControlSettings(UI_RESOURCE_DIR "/options/ColorPicker.res");
	InvalidateLayout();
	MakePopup();
	SetVisible(false);	// Hide by default

	SetInitialColor(SDK_Color(127, 255, 0, 255));	// Reset color
}
...
void CColorPicker::SetColor(SDK_Color color)
{
	float h, s, v;
...
void CColorPicker::SetInitialColor(SDK_Color color)
{
